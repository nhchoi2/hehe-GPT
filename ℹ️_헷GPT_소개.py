...
st.write(
    ...
    "이 애플리케이션은 개발자, 교육자, 학생 등 코딩 학습과 테스트를 필요로 하는 모든 이들에게 유용한 도구를 제공합니다."
)
...
st.header("사용 방법")
st.markdown("""
1. **코드 입력:** 메인 페이지의 입력 창에 실행하고자 하는 Python 또는 HTML 코드를 입력합니다.
2. **코드 실행:** 입력을 완료하면 자동으로 코드가 실행되며, 결과가 화면에 표시됩니다.
3. **결과 확인:** 코드 실행 결과 또는 렌더링된 HTML 페이지를 실시간으로 확인할 수 있습니다.
""")

# 개발 동기
st.header("개발 동기")
st.write(
    "코딩을 학습하거나 새로운 아이디어를 테스트할 때, 별도의 개발 환경을 설정하는 것은 번거로울 수 있습니다. "
    "헷GPT는 이러한 불편을 해소하고자, 웹 브라우저만으로도 손쉽게 코드를 실행하고 결과를 확인할 수 있는 환경을 제공하기 위해 개발되었습니다."
)
